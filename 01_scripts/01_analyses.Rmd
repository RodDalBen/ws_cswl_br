...
* Joint vs. single task: Here we use data from other experiments with a single task.
...
plot_ws_only <- 
  ...
  facet_wrap(~ speech_version_label) +
  annotate("segment", x = -Inf, xend = Inf, y = 0.5, yend = 0.5, linetype = "dashed", alpha = 0.6) + # chance level ws
  labs(title = "Segmentation only vs. parallel experiment", y = "Word selection", x = "Experiment") + 
  theme_classic(base_size = 10)
...
#ggsave(plot = plot_perf_map_freq_exp1, 
...
plot_cswl_only <- 
  ...
  labs(title = "Mapping only vs. parallel experiment", y = "Object selection") + 
  theme_classic(base_size = 10) +
  theme(axis.title.x = element_blank(),
        axis.text.x = element_blank(),
        axis.ticks.x = element_blank())
...
# Exp 2 speech segmentation performance and self-evaluation
...
# joint plot
plot_joint_seg_exp2 <- 
  plot_perf_seg_exp2 + plot_corr_seg_eval_exp2 + 
...
#ggsave(plot = plot_map_freq_exp2, 
...
plot_corr_seg_map_exp2 <- 
  ...
  coord_cartesian(ylim = c(0, 1), xlim = c(0, 1)) +
  scale_size_area() +
  labs(title = "Experiment 2 - Correlation between \nsegmentation and mapping",
       y = "Word selection", x = "Object selection") + 
  theme_classic(base_size = 10) +
...
mod_joint_ws_conflict <- 
  ...
  filter(experiment %in% c("ws only", "ws + cswl joint"), 
         ...
         trial_type == "test ws") %>%
  lme4::glmer(is_correct ~ experiment + (experiment|p_n_unique), # convergence warning, but results don't change: keeping both random slope (experiment) and intercept (participant), instead of only participant
              data = .,
              family = "binomial")
